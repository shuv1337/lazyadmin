...
- The UX-overhaul shared projection layer starts in `crates/lazyadmin-runtime/src/view_model/` (`digest`, `doctor_groups`, `header_pip`, `inspector`, `search`). Keep new TUI/Web grouping and summary logic there instead of duplicating snapshot projections in each UI.
