...
This repository now contains the v0.4 Rust workspace for `lazyadmin` after PLAN-01 through PLAN-15, PLAN-24, and the architecture-deepening follow-ups:

- `Cargo.toml` — workspace root (`resolver = "2"`) with shared dependencies.
- `crates/lazyadmin-core` — core models, graph/discovery contracts, reusable action plans/dry-run rendering, config loader, redaction, selectors, snapshot/diff JSON contract, and telemetry primitives.
...
- `crates/lazyadmin-runtime` — shared snapshot/event assembly plus renderer-agnostic view-models for digest, doctor groups, header health, inspector, search, listener tables, relation lookups, and live snapshot feeds.
- `crates/lazyadmin-tui` contains the Ratatui interface: responsive rendering of runtime view-models, Process Tree and Metrics views, theme/keybinding support, terminal panic guard, and interaction state. View/render tests cover 120/90/70/50 column modes; avoid launching the TUI interactively in automation and prefer `lazyadmin tui --headless --json`.
...
- `crates/lazyadmin-core` for normalized graph models, correlation, action plans/dry-run rendering, config, redaction, JSON schemas, diffs, and telemetry primitives.
...
- `crates/lazyadmin-runtime` for shared snapshot/event assembly and renderer-agnostic view-models.
...
- Pure free-port planning lives in `lazyadmin_core::actions`; host mutation does not. Keep live rescans, process-key revalidation, confirmation prompts, signals, manager APIs, and post-action verification in CLI/runtime layers. Inspector action previews should use the first line of `lazyadmin-core::actions` dry-run output when an `Action` exists, and only use hand-written strings for UI-only affordances such as logs/editor links.
...
- The Web UI static app is now three files — `crates/lazyadmin-web/static/index.html`, `app.css`, `app.js` — each embedded via `include_str!` and served with `Cache-Control: no-store` in dev. There is no bundler, no framework, and no inline JS. Read-only API surface: `/api/health`, `/api/snapshot`, `/api/digest`, `/api/doctor`, `/api/header_pip`, `/api/inspector?kind=&id=`, `/api/search`, `/api/rail`, `/api/views/overview`, `/api/views/listeners`, `/api/entities/:kind/:id`, `/api/events`. The Web UI nav is the same `RAIL_ENTRIES` constant the TUI rail consumes, grouped client-side into Triage / Inventory / Diagnostics.
- Global search lives in `lazyadmin_runtime::view_model::search` and serializes `lazyadmin.search.v1` for CLI, Web, and TUI. The TUI launches with `InputMode::Search` focused while still rendering Overview until the first query character; do not reintroduce footer-only filter prompts.
...
- The UX-overhaul shared projection layer lives in `crates/lazyadmin-runtime/src/view_model/` (`digest`, `doctor_groups`, `header_pip`, `inspector`, `listeners`, `relations`, `search`). Keep new TUI/Web grouping, relation lookup, listener-row, and summary logic there instead of duplicating snapshot projections in each UI.
- Live refresh ownership lives in `lazyadmin-runtime/src/live.rs`. CLI, TUI, and Web should consume `spawn_live_snapshot_feed` instead of each adding a separate snapshot/event loop. DiscoveryEvent messages remain hints; polling snapshots remain authoritative.
...
- PLAN-15 #19 rail collapse is landed: the canonical rail order lives in `lazyadmin_runtime::view_model::RAIL_ENTRIES` (`Overview`, `Listeners`, `Workloads`, `Processes`, `Doctor`, `Metrics`), and both TUI and Web consume it. Legacy filtered views such as `Public`, `Conflicts`, and `Orphans` remain programmatically addressable but are hidden from the rail.
...
- `cargo test --workspace -- --list` currently lists **427 tests across all crates**; the normal workspace run passes with the optional live Linux integration test ignored unless explicitly enabled.
